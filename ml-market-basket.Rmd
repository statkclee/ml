...
## 데이터프레임 &rarr; 장바구니 데이터 [^market-basket-dataframe] {#dataframe-to-market-basket-data}
...
cart_txt %>% 
    tbl_df() %>%
    rename(groceries_in_cart=value) %>% 
    sample_n(10) %>% 
    DT::datatable()
```
...
두가지 작업 흐름이 있지만 데이터프레임을 `.csv` 파일로 떨어뜨리고 이를 `read.transactions()` 함수로 불러오는 경우를 살펴본다.

``` {r ml-groceries-dataset-df-long}
order_df <- tribble(
...
## 장바구니 데이터 불러오기 {#market-basket-data-import}
