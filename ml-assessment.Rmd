...
<center>
<img src="fig/precision-recall.png" alt="정밀도와 재현율 도식화" width="50%"> [^wiki-walber]
</center>
...
* 재현율(Recall) 혹은 민감도(Sensitivity): 예를 들어, 환자가 실제 암이 있는데, 양성으로 검진될 확률. 연관된 항목이 얼마나 많이 선택되었는지 측정. Sensititivity는 Hit Rate로 알려져 있다.
...
실무에서는 recall(재현율)과 sensitivity(민감도)는 상호 교차하여 사용되지만 엄밀하게는 동일한 개념은 아니다.

- Precision and Recall 은 양성을 얼마나 예측하는지 초점을 맞췄다.
- Sensitivity and Specificity는 양성과 음성을 정확하게 분류하는데 초점을 맞췄다.
