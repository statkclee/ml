...
## 1. 탐색적 다차원 데이터 분석 - 포켓몬 데이터 [^prcatical-guide-to-cluster-analaysis-in-r] {#pokemon-data-multivariate}
...
### 1.1. 군집분석 작업흐름 {#clustering-pipeline}
...
## 2. 군집분석 {#pokemon-cluster-analysis}

### 2.1 포켓몬 데이터 불러와서 전처리 작업 {#cluster-import-preprocess-pokemon-data}
...
### 2.2 군집분석을 위한 탐색적 데이터 분석 {#cluster-eda-pokemon-data}
...
### 2.2.1. 군집(cluster)은 존재하는가? {#cluster-eda-exist-cluster}
...
### 2.2.2. 군집(cluster)은 몇개가 적절할까? {#cluster-eda-how-many-cluster}
...
### 2.3. 최적 군집분석 알고리즘 선정 {#cluster-eda-optimal-algorithm-cluster}
...
### 2.4. 군집분석 알고리즘 적합 {#cluster-eda-fit-algorithm-cluster}
...
### 2.5. 군집분석 알고리즘 - 열지도(heatmap) {#cluster-eda-fit-algorithm-cluster-heatmap}
...
## 3. 군집특성 요약 {#cluster-summary-cluster}

군집 알고리즘을 통해 해당 관측점에 대한 군집에 대한 배정이 완료되었다면,
이제는 해당 군집별 특성을 요약하는 작업을 수행한다.

### 3.1. 군집특성 요약 데이터 {#cluster-summary-cluster-cbind}
...
### 3.2. 군집특성 요약 데이터 - 병렬그래프 {#cluster-summary-cluster-parallel}

병렬그래프(parallel plot)를 통해 군집을 잘 식별하는 변수가 어떤 것인지 활용한다.

``` {r cluster-summary-cluster-parallel}
# 2. 포켓몬 군집 해석 ------------------
## 2.1. 전체적인 능력값
pkmon_full_df %>% 
    mutate(ID = 1:n()) %>% 
...
### 3.3. 군집특성 요약 데이터 - 모자이크 그래프 {#cluster-summary-cluster-mosaic}

군집은 태생이 범주형인데, 범주형 변수가 포함된 경우 군집과 교차하여 군집별 특성도 함께 살펴본다.
이를 위해서 모자이크 그래프를 활용한다.

``` {r cluster-summary-cluster-mosaic}
## 2.2. 범주형 변수에 대한 군집
pkmon_full_df %>% 
...
### 3.3. 군집특성 요약 데이터 - 밀도 그래프 {#cluster-summary-cluster-mosaic}
