...
``` {r hq-corpus}
book <- readLines("03_data/저녁이 있는 삶.txt", encoding="UTF-8")
...
ggplot(data.frame(term = names(book_term_freq), freq = book_term_freq), aes(term, freq)) + 
...
ggplot(book_term_df, aes(reorder(term, freq), freq)) + 
  geom_bar(stat="identity") + coord_flip() +
  ylab("단어 빈도수") +
  xlab("")
